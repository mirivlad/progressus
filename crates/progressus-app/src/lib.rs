...
pub use progressus_sim::{
    ...
    ItemId, ItemLocation, ItemQuantity, JobKind, JobState, KnowledgeId, LocalCell,
    MAX_PRODUCTION_ORDER_RUNS, MAX_REST, MAX_SATIETY, MAX_SKILL_PRACTICE, MovementSpeed,
    MovementState, NaturalResource, NaturalResourceId, ProductionLogistics, ProductionOrder,
    ProductionTarget, ProductionZoneKind, RESIDENT_CHUNK_RADIUS, RESIDENT_CHUNKS_PER_CENTER,
    RecipeId, SAVE_FORMAT_VERSION, SUBUNITS_PER_CELL, SaveError, SaveMetadata, SimulationTick,
    SkillId, SlotId, Stockpile, Structure, StructureId, TerrainId, Workstation,
    WorkstationConstructionSite, WorkstationId, WorldCell, WorldPosition, WorldSeed,
    WorldgenVersion, item, knowledge, natural_resource, recipe, skill, slot, structure, terrain,
    workstation,
};
pub use read_model::{
...
impl Application {
    ...
    pub fn snapshot(&self, mut query: SnapshotQuery) -> Result<ClientSnapshot, ApplicationError> {
        ...
        Ok(ClientSnapshot {
            ...
            workstations,
            production_orders,
            known_knowledge: self.simulation.known_knowledge().collect(),
            production_logistics,
            construction_sites,
            workstation_construction_sites,
            structures,

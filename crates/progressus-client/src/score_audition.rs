...
fn add_short_key(pcm: &mut [f32], event: NoteEvent, gain: f32, pan: f32) {
    let (first, last, start_sample) = sample_bounds(event.start, event.duration, pcm.len() / 2);
    let frequencies = felt_string_frequencies(event.midi);
    let (left, right) = pan_gains(pan);
    for frame in first..last {
        let time = (frame as i64 - start_sample) as f32 / SAMPLE_RATE as f32;
        let attack = (time / 0.012).min(1.0);
        let release = ((event.duration - time) / 0.14).clamp(0.0, 1.0);
        let decay = (-time / 0.55).exp();
        let mut tone = 0.0;
        for frequency in frequencies {
            let phase = TAU * frequency * time;
            tone += phase.sin() * 0.68
                + (phase * 2.0).sin() * 0.23 * (-time / 0.26).exp()
                + (phase * 3.0).sin() * 0.09 * (-time / 0.13).exp();
        }
        let hammer = (stateless_unit(event.midi.to_bits() as u64, frame as u64) * 2.0 - 1.0)
            * (-time / 0.025).exp()
            * 0.04;
        let sample = (tone / 3.0 + hammer) * attack * release * decay * gain;
        pcm[frame * 2] += sample * left;
        pcm[frame * 2 + 1] += sample * right;
    }
}

fn next_diatonic(midi: f32) -> f32 {
    const NOTES: [f32; 11] = [59., 60., 62., 64., 65., 67., 69., 71., 72., 74., 76.];
    NOTES
        .iter()
        .copied()
        .find(|note| *note > midi)
...
fn audition_pcm(seed: u64, seconds: usize, arrangement: AuditionArrangement) -> Vec<f32> {
    let plan = collect_plan(seed, seconds as f32);
    let mut pcm = vec![0.0; seconds * SAMPLE_RATE as usize * 2];
    let step: f32 = match arrangement {
        AuditionArrangement::Flowing => 1.55,
        AuditionArrangement::Lively => 0.92,
        AuditionArrangement::CallAndResponse => 1.85,
    };
    for (region_index, region) in plan.regions.iter().enumerate() {
        let end = plan
            .regions
            .get(region_index + 1)
            .map_or(seconds as f32, |next| next.start)
            .min(seconds as f32);
        let mut at = region.start;
        let mut note_index = 0;
        while at < end {
            let chord = region.chord;
            let midi = match arrangement {
                AuditionArrangement::Flowing => [
                    chord[0],
                    next_diatonic(chord[0]),
                    chord[1],
                    chord[2],
                    next_diatonic(chord[1]),
                    chord[1],
                ][(note_index + region_index) % 6],
                AuditionArrangement::Lively => {
                    [
                        chord[0],
                        chord[2],
                        chord[1],
                        next_diatonic(chord[2]),
                        chord[0],
                        next_diatonic(chord[0]),
                        chord[2],
                        chord[1],
                    ][(note_index + region_index) % 8]
                        + if note_index % 8 == 3 { 12.0 } else { 0.0 }
                }
                AuditionArrangement::CallAndResponse => {
                    [chord[0], next_diatonic(chord[0]), chord[1], chord[2]]
                        [(note_index + region_index) % 4]
                }
            };
            add_short_key(
                &mut pcm,
            ...
            if matches!(arrangement, AuditionArrangement::CallAndResponse) && at + 0.52 < end {
                add_short_key(
                    &mut pcm,
                    NoteEvent {
                        start: at + 0.52,
            ...
            at += step
                * if (note_index + region_index) % 5 == 4 {
                    1.45
                } else {
                    1.0
                };
    ...
    for phrase in &plan.phrases {
        for (index, note) in phrase.notes.iter().enumerate() {
            ...
            let start = phrase_start + (note.start - phrase_start) * 0.62;
            add_short_key(
                &mut pcm,
                NoteEvent {
                    start,
                    midi: note.midi,
                    duration: note.duration.min(0.72),
                },
                0.12,
                -0.18 + (index % 4) as f32 * 0.12,
            );
        }
    }
    apply_room(&mut pcm);
...
pub fn audition_wav(seed: u64, seconds: usize, arrangement: AuditionArrangement) -> Vec<u8> {
    encode_wav(&audition_pcm(seed, seconds, arrangement))
}

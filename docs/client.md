...
- Orders, Zones and Build open grouped tool palettes. Harvest, stockpile editing, wall and cancellation tools support rectangular drag selection with cell grid feedback. Door, Bed and workbench use point placement. A wall, door, bed or workbench may be designated on passable ground occupied only by a source, loose stack, or person; the preparation job clears that occupancy physically before completion.
...
Stage B adds a low-poly Bed and a seated sleeping/breathing pose driven only by the detached Sleep job. The Build palette exposes Bed with shortcut B. Headless and client tests cover the authoritative job and presentation mapping. A native frame confirmed the palette label and bed geometry; the exact click-to-designate sequence and live sleeping pose still require user-side visual acceptance.

...
- Orders, Zones and Build open grouped tool palettes. Harvest, Rock excavation, stockpile editing, wall and cancellation tools support rectangular drag selection with cell grid feedback. Rock excavation submits only known Rock cells, requires a mining-capable physical tool, changes each completed cell to Grass and leaves physical Stone for ordinary hauling; its preview and job marker sit above the raised rock mesh. Door, Bed, workbench and furnace use point placement. A wall, door, bed or workstation may be designated on passable ground occupied only by a source, loose stack, or person; the preparation job clears that occupancy physically before completion.
- Character inspector shows identity, location, satiety, rest, last completed sleep's shelter verdict, movement, carried goods, current job/state and localized Gathering/Mining/Crafting practice (0–5).
...
Stage B adds a low-poly Bed and a lying sleeping/breathing pose driven only by the detached Sleep job. The Build palette exposes Bed with shortcut B. Headless and client tests cover the authoritative job and presentation mapping. Native bed designation, completed geometry, and the live sleeping pose were observed on 2026-09-26; owner aesthetic acceptance remains open.

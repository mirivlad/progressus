...
- Models that stand on the ground must meet it with a footprint, which a test now measures as the width of the mesh's cross-section at ground level rather than as the spread of its vertices. A `gem` is a bipyramid with no vertices near its base, so a vertex-only check would have missed exactly the defect that prompted it: an outcrop centred at its own vertical radius balanced on its lower apex and read as hovering over its own shadow. The same check found loose wood floating a full beam-width above the ground, a tool hovering, and characters standing three centimetres clear — all older than this change.

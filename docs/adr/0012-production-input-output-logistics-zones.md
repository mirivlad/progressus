...
Production objects may impose their own port geometry on top of the shared zone ownership model. The current one-cell Workbench has exactly two cardinal Input ports and exactly two diagonal Output ports. Each pair has six canonical unordered layouts and rotates independently from the workstation modal. The player does not paint Workbench Input or Output cells individually. Cardinal inputs and diagonal outputs cannot consume one another's positions. Future multi-cell producers may define different counts and perimeter rules without changing `ProductionLogistics` ownership.

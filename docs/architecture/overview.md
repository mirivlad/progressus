...
`progressus-app` exposes stockpile cell/policy commands, detached stockpile snapshots including disallowed item kinds, and carried-item snapshots. The Bevy client keeps zone editing in a compact Zones palette rather than a permanently flat toolbar. Visible stockpiles use a translucent ~30% ground overlay above terrain but below physical entities; permanent internal cell borders are removed, a selected stockpile gets only its external boundary, and the HUD can hide/show the zone layer. In Select mode a stockpile can be selected, explicitly configured from its inspector, or opened by double-click. The stockpile modal presents category tri-state controls and concrete item toggles backed by the authoritative policy. Carried item sprites remain parented to worker visuals so transport stays visible through interpolation. Ctrl+left-click remains a temporary single-cell shortcut. Internal exact stack splitting exists for production supply and meal allocation, but player-directed/general-purpose splitting is not exposed. Stockpile priorities, containers, skills, and generalized job policy remain later work.

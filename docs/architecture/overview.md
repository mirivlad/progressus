...
`progressus-app` exposes stockpile commands/snapshots and carried-item snapshots. The Bevy bootstrap exposes `Stockpile +` and `Stockpile -` toolbar tools with rectangular drag designation, outlines stockpile cells, and parents carried item sprites to worker visuals so transport remains visible through presentation interpolation. Ctrl+left-click remains a temporary single-cell shortcut. This remains a bootstrap: stockpile filters/priorities, stack splitting, containers, construction, crafting, skills, and generalized job policy are later work.

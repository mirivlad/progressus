...
`progressus-app` exposes `DesignateHarvest`/`CancelJob`, detached job snapshots, and a job revision. The Bevy bootstrap maps Shift+left-click on a natural source to toggle the harvest designation and draws a small state-colored bracket around designated sources.

### Current stockpile/haul bootstrap

`progressus-sim` also owns deterministic stockpile areas and `Haul` jobs. A stockpile is a stable-ID set of discovered walkable ground cells with a unique cell-owner index. Eligible ground stacks outside all stockpiles receive deterministic haul jobs only when a free destination cell exists. Job indexes reserve one item, one destination cell, and one worker; the worker travels to the exact item position, picks up the canonical stack, carries it to the destination cell center, and drops the same stable stack there. Manual interruption or removal of an active destination releases reservations and drops a carried stack at the worker's current position.

`progressus-app` exposes stockpile commands/snapshots and carried-item snapshots. The Bevy bootstrap uses Ctrl+left-click to add/remove cells from the primary stockpile, outlines stockpile cells, and parents carried item sprites to worker visuals so transport remains visible through presentation interpolation. This remains a bootstrap: stockpile filters/priorities, stack merging, capacity, containers, construction, crafting, skills, and generalized job policy are later work.

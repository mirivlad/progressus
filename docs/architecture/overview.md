...
### Current rest/sleep/shelter bootstrap

Each character also owns persistent rest, initially `100`, decaying by one every 48 authoritative ticks. At `<= 30`, an exclusive `Sleep` job reserves a reachable completed Bed (a passable physical `2 Wood` construction) or uses the character's current ground cell if no bed is available. Hunger and direct orders cancel Sleep through the ordinary job lifecycle and release bed ownership. Completion restores rest to full for a bed in an enclosed space, adds 50 for an open bed, or adds 25 on the ground. A bounded 256-cell cardinal flood fill derives enclosure from completed walls/doors and non-walkable terrain at completion time; it does not materialize chunks, reveal terrain, persist rooms, or include furniture/blueprints as boundaries. Rest, active Sleep and the last completed shelter verdict round-trip through save v1. `progressus-app` publishes detached rest/job/verdict snapshots, while Bevy only presents the Bed build tool, mesh and inspector; it never decides shelter. See [`ADR-0020`](../adr/0020-rest-sleep-and-enclosure-shelter.md).

### Current workbench/craft bootstrap

`progressus-sim` owns stable-ID workstations, stable-ID production orders, a `ProductionLogisticsWorld`, and a registry-defined primitive-tool recipe (`2 Wood + 1 Stone -> 1 PrimitiveTool`, six work ticks). A production order has an explicit `Finite { remaining_runs }` or `Infinite` target. Only one Craft job can occupy a workstation at a time; pending orders are considered in creation order. Each producer owns explicit Input and Output ground-cell zones with unique cell ownership. The current one-cell Workbench specializes that generic ownership model into exactly two cardinal Input ports plus exactly two diagonal Output ports. Input and Output pairs each cycle independently through six canonical unordered layouts and neither can be edited cell-by-cell through the public simulation API. These cells cannot simultaneously be ordinary stockpile ground. A Craft job reserves concrete physical stacks only from its own Input zone. Missing quantities are split exactly from eligible ordinary stockpile stacks and carried by a dedicated `SupplyProduction` job into a reserved Input cell. Completion consumes exact quantities, preserving partial stack IDs or removing exhausted stacks, creates the physical `PrimitiveTool` in the producer's Output zone, and decrements only finite targets. Ordinary Haul can then move the output from Output to storage. Shared physical inputs cannot be double-reserved; a two-infinite-order regression verifies deterministic shared-source arbitration without starvation. See [`ADR-0012`](../adr/0012-production-input-output-logistics-zones.md).

`progressus-app` exposes workstation, production-order, and production-zone commands, detached workstation/order/logistics snapshots, and authoritative workstation/production/logistics revisions. The Bevy toolbar exposes Workbench and Furnace as point placement tools. Clicking an existing workstation opens a reusable `ModalKind` shell even when another build/designation tool remains active; the active tool is not cleared, so closing the inspector returns directly to that workflow. The workstation view edits authoritative production orders, including an explicit `∞` control; removing a workstation is also handled from that modal. The modal logistics schematic uses the corresponding procedural workstation image, marks the two physical Input ports in red and the two physical Output ports in yellow, and exposes separate localized rotation actions rather than logistics paint tools. The same modal shell is intended for later containers, research stations, and other inspectors. Shared UI strings use a typed localization layer with Russian as the default language and English as the second built-in language. Bevy's embedded ASCII-only font is not sufficient for Russian, so the bootstrap resolves a local system font with Cyrillic coverage. See [`ADR-0010`](../adr/0010-production-orders-and-localized-modal-ui.md). Workstation placement is immediate on a clear cell; removable center-cell occupancy creates a stable-ID deferred preparation project which is persisted and published separately until it can become the workstation.

Prototype 02 adds a Furnace through the same workstation, order, reservation, and fixed-port contracts. Its recipe consumes physical `2 CopperOre + 1 Wood` (wood is the current fuel) over 24 base work ticks and creates one physical `CopperIngot` at an output port. The client gives the furnace and ingot separate procedural models and localized names; the workstation modal selects recipes by workstation kind.

The Prototype 02 metallurgy study is a one-time Workbench recipe that physically moves one CopperOre sample from Input to Output while performing work. Successful completion adds a stable-name `metallurgy` entry to the simulation's persistent settlement knowledge set; the copper-ingot recipe declares that entry as a prerequisite. Authoritative order admission enforces the prerequisite, and the client displays the locked reason from the detached knowledge snapshot. The knowledge set never substitutes for ore, wood fuel, the furnace, work, or output capacity.

### Current physical-construction bootstrap

`progressus-sim` owns a stable-ID `ConstructionWorld` for unfinished sites and completed structures. `structure::STONE_WALL` costs 2 Stone and fixed work. A site reserves one concrete compatible stack, excludes it from competing Haul/Craft use, and creates a `DeliverConstruction` job. The worker physically picks up that canonical stack and drops it at a deterministic reachable work position beside the site; only a delivered stack can enable `Construct`. Completion consumes exactly 2 Stone and converts the site to a finished structure with the same stable ID. A partial remainder stays on the ground beside the wall. Cancellation drops carried material before clearing jobs/reservations.

Finished structure occupancy is separate from terrain. `StoneWall` blocks movement and A*, while `Door` is passable with navigation cost 2 versus ordinary ground cost 1. Door open/closed state is automatic and authoritative: occupancy opens it and a short deterministic hold window closes it after passage. Unfinished blueprints remain passable. Door designation may replace either a planned StoneWall (through normal cancellation/cleanup) or a completed StoneWall (removing that occupancy before creating the Door site); this narrow conversion does not add general demolition or salvage. Stone walls and doors share the same presentation connectivity network, and the door frame follows the predominant neighboring wall axis (east/west on ties). The Bevy client exposes Stone wall plus Door in the Build palette; Cancel-jobs removes unfinished sites but does not demolish completed structures. See [`ADR-0009`](../adr/0009-physical-construction-sites-and-blocking-structures.md).
...
`progressus-sim` owns a deterministic `ItemWorld` keyed by the same global stable `EntityId` space as characters. Current item definitions include Wood, Stone, PrimitiveTool, Berries, CopperOre and Cart. Every stack has a quantity in `1..=1024` and exactly one canonical location: Ground, Carried, Equipped or Contained. Ground items are indexed by chunk, carried/equipped items by character and contents by container; transfer operations update the canonical stack and its indexes atomically. Character interaction reach uses integer `InteractionRadius` geometry, and dropping is rejected onto non-walkable effective terrain.

`progressus-app` exposes only explored ground items in requested chunks as detached `GroundItemSnapshot` values plus an item revision. Natural-resource snapshots are likewise chunk-scoped and contain only explored currently-available source cells; a separate resource revision covers authoritative depletion and renewable regrowth visibility changes. Carried stacks are published as detached carrier-linked snapshots. `InventoryItemSnapshot` additionally publishes a stack's physical location, whether a job holds it, what it costs to carry and — for a container — its capacity and the load already inside it. Those rows follow the goods rather than the camera: a stack borne by a character travels with them and is published whatever chunks were asked for, while a stack resting on the ground, including one inside a parked container, stays subject to exploration and the requested chunks, so packing something away never reveals it. `Command::{PickUpItem, DropItem, EquipItem, UnequipItem, LoadContainer, UnloadContainer}` carry player intent the other way; each delegates to the matching authoritative operation, and `DropItem` puts goods down where the character stands rather than anywhere the player names. The Bevy client reconciles disposable ground-item entities by stable Progressus ID and converts their exact positions relative to the nearby render origin, just like characters.

Harvested resources create ordinary physical ground stacks, including Berries from renewable BerryBush sources, and the haul bootstrap can move those stacks through the canonical `Carried` state into designated stockpile cells. A stockpile is intentionally not a container: delivered stacks remain `Ground` at exact positions inside the stockpile area, as recorded by [`ADR-0006`](../adr/0006-stockpiles-remain-physical-ground.md). Haul uses a merge-first destination policy: a compatible underfilled same-kind stack is preferred over an empty stockpile cell when the combined quantity is at most 1,024. Underfilled stacks already inside the same stockpile are also valid Haul sources and consolidate toward a lower stable-ID target, preventing ping-pong; the destination stack keeps its stable ID and the emptied source stack is removed. Exact internal stack splitting exists for carrying limits, production supply and per-character meal portions. Containers retain their physical contents when parked; there is still no player-directed/general-purpose splitting UI; physical stacks and active logistics round-trip through save format v1, while raw generated chunks use bounded derived residency. Crafting, StoneWall construction, and autonomous eating now provide explicit quantity-consumption paths over physical stacks.
...
- `progressus-content` owns what exists in the game as data: items, terrain, natural resources, structures, workstations and recipes. It holds no authoritative state, no coordinates and no Bevy, so every layer above it shares one vocabulary. Each definition carries a stable lowercase name that saves store and localization keys off; the runtime handle is a registry index and is never persisted. Behavior reads definition fields — nutrition makes an item food, a regrowth delay makes a resource renewable, a navigation cost makes a structure passable — so adding content does not mean teaching a system a new name. Registries are append-only because handle order decides map iteration order. See [`ADR-0021`](../adr/0021-content-registry.md).
- `progressus-worldgen` owns deterministic terrain generation and coordinate types. A world carries one pinned base version — its terrain plus the resources it was created with — and a set of additive resource layers applied over that base. The base answers first; layers only fill cells nothing else claimed, in registry order, so an update may add resources to an existing world but can never move or remove what that world already has. Terrain is never layered. A save records its active layers and loading applies every layer the build knows, refusing a save that names one this build lacks. See [`ADR-0022`](../adr/0022-additive-worldgen-layers.md).
...
- A container holds goods of its own. `ItemLocation` covers the ground, the hands, an equipment slot and the inside of a container, and a stack sits in exactly one of them. A container's capacity is a multiple of one pair of hands, so a cart at four holds forty wood or twenty stone through the same fraction rule. Contents belong to the container rather than to whoever is pushing it, which is what lets a loaded cart be set down. Containers do not nest more than two deep and may not enclose themselves, and goods leave a container for the ground directly, because a cartload could never fit through a pair of hands.
- Carrying and equipment are physical. Each item declares how many of it one pair of hands holds, and a mixed load shares those hands as an integer fraction sum. Equipment lives in named slots, does not count against carrying, and is what grants a capability: systems ask whether a character can perform the work, never which slot holds what, so a new slot costs one registry row. Extraction that requires a capability is only assigned to someone equipped for it, and a fetch job puts a tool in a slot when nobody is. See [`ADR-0024`](../adr/0024-character-inventory-and-containers.md).
...
- `progressus-client` is a native Bevy presentation consumer. It launches seed `0` by default or a requested `--seed`, owns pause timing/save-slot files/localization/selection/inspectors, the icon-first HUD and its Orders/Zones/Build palettes, zone-layer visibility, tooltips, and stockpile/workstation modal presentation, and renders only detached application read models. Its only direct dependencies are Bevy, `progressus-app` and `progressus-content`; it must not directly depend on `progressus-sim` or `progressus-worldgen`.

The headless application chain is Bevy-free. The boundary guard scans that complete chain and verifies the client's direct-dependency rule. `progressus-content` is allowed there because it is shared vocabulary rather than state or authority. The Bevy client converts `ClientSnapshot` data into disposable entities mapped from stable Progressus IDs; those mappings are derived presentation caches, never authoritative or persistent state.

...
`progressus-sim` also owns deterministic stockpile areas and `Haul` jobs. A stockpile is a stable-ID set of discovered walkable ground cells with a unique cell-owner index. Eligible ground stacks outside all stockpiles receive deterministic haul jobs when a destination cell is either empty or contains a same-kind stack with enough remaining capacity. Job indexes reserve one item, one destination cell, and one worker; the worker travels to the exact item position, picks up the canonical stack, carries it to the destination cell center, and drops the same stable stack there. Manual interruption or removal of an active destination releases reservations and drops a carried stack at the worker's current position.

`progressus-app` exposes stockpile commands/snapshots and carried-item snapshots. The Bevy bootstrap exposes `Stockpile +` and `Stockpile -` toolbar tools with rectangular drag designation, outlines stockpile cells, and parents carried item sprites to worker visuals so transport remains visible through presentation interpolation. Ctrl+left-click remains a temporary single-cell shortcut. This remains a bootstrap: stockpile filters/priorities, stack splitting, containers, skills, and generalized job policy are later work.

### Current workbench/craft bootstrap

`progressus-sim` owns stable-ID workstations, stable-ID production orders, and a typed `RecipeId::PrimitiveTool` definition (`2 Wood + 1 Stone -> 1 PrimitiveTool`, six work ticks). A production order has an explicit `Finite { remaining_runs }` or `Infinite` target. Only one Craft job can occupy a workstation at a time; pending orders are considered in creation order. A Craft job exclusively reserves its worker, workbench, order, and concrete physical input stack IDs. Inputs are eligible only while lying on designated stockpile ground within Manhattan distance 1 of the workbench; no global inventory counter or map-wide deduction is allowed. Completion consumes exact quantities from those stacks, preserving a partially consumed stack ID or explicitly removing a fully consumed stack, creates a physical `PrimitiveTool` stack beside the workbench, and decrements only finite targets. Ordinary Haul may then move or merge that output under the normal logistics rules. Shared physical inputs cannot be reserved by two workstations at once; a regression scenario with two infinite orders verifies that an already-waiting workstation receives the next shared input rather than starving behind a newly recreated job.

`progressus-app` exposes workstation and production-order commands, detached workstation/order snapshots, and authoritative workstation/production revisions. The Bevy toolbar keeps Workbench only as a point placement tool. In Select mode, clicking a workbench opens a reusable `ModalKind` shell whose workstation view edits authoritative production orders, including an explicit `∞` control; removing a workbench is also handled from that modal. The same modal shell is intended for later containers, furnaces, research stations, and other inspectors. Shared UI strings use a typed localization layer with Russian as the default language and English as the second built-in language. Bevy's embedded ASCII-only font is not sufficient for Russian, so the bootstrap resolves a local system font with Cyrillic coverage. See [`ADR-0010`](../adr/0010-production-orders-and-localized-modal-ui.md). Workbench placement remains an instantaneous bootstrap exception; physical StoneWall construction is implemented separately through construction sites and delivery/construct jobs.
...
Prototype 01 uses only a few recipes. The architecture should avoid hard-coding each product's behavior into bespoke UI or simulation classes. The first implemented typed recipe is `PrimitiveTool`: two Wood plus one Stone at a Workbench, consuming concrete nearby stockpile stacks and creating one physical output.

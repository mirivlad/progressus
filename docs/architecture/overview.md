...
`progressus-app` exposes stockpile commands/snapshots and carried-item snapshots. The Bevy bootstrap exposes `Stockpile +` and `Stockpile -` toolbar tools with rectangular drag designation, outlines stockpile cells, and parents carried item sprites to worker visuals so transport remains visible through presentation interpolation. Ctrl+left-click remains a temporary single-cell shortcut. Internal exact stack splitting now exists for production supply and meal allocation, but player-directed/general-purpose splitting is not exposed. Stockpile filters/priorities, containers, skills, and generalized job policy remain later work.

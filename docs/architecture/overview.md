...
`progressus-app` exposes stockpile commands/snapshots and carried-item snapshots. The Bevy bootstrap exposes `Stockpile +` and `Stockpile -` toolbar tools with rectangular drag designation, outlines stockpile cells, and parents carried item sprites to worker visuals so transport remains visible through presentation interpolation. Ctrl+left-click remains a temporary single-cell shortcut. This remains a bootstrap: stockpile filters/priorities, stack splitting, containers, physical construction, skills, and generalized job policy are later work.

### Current workbench/craft bootstrap

`progressus-sim` owns stable-ID workstations and a typed `RecipeId::PrimitiveTool` definition (`2 Wood + 1 Stone -> 1 PrimitiveTool`, six work ticks). A Craft job exclusively reserves its worker, workbench, and the concrete physical input stack IDs. Inputs are eligible only while lying on designated stockpile ground within Manhattan distance 1 of the workbench; no global inventory counter or map-wide deduction is allowed. Completion consumes exact quantities from those stacks, preserving a partially consumed stack ID or explicitly removing a fully consumed stack, and creates a new physical `PrimitiveTool` stack beside the workbench. Ordinary Haul may then move or merge that output under the normal logistics rules.

`progressus-app` exposes workstation/craft commands, detached workstation snapshots, and a workstation revision. The Bevy toolbar exposes point tools for Workbench placement/removal and toggling the current primitive-tool craft order. Workbench placement is intentionally instantaneous bootstrap placement rather than construction; material delivery to a build site and Construct jobs remain later work.
...
Harvested resources create ordinary physical ground stacks, and the haul bootstrap can move those stacks through the canonical `Carried` state into designated stockpile cells. A stockpile is intentionally not a container: delivered stacks remain `Ground` at exact positions inside the stockpile area, as recorded by [`ADR-0006`](../adr/0006-stockpiles-remain-physical-ground.md). Haul delivery merges a compatible same-kind ground stack when the combined quantity is at most 1,024; the destination stack keeps its stable ID and the emptied source stack is removed. There is still no real container/storage location, stack splitting, physical construction, persistence, or residency. Crafting now supplies the first explicit quantity-consumption path.

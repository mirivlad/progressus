...
Finished structure occupancy is separate from terrain. `StoneWall` blocks movement and A*, while `Door` is passable with navigation cost 2 versus ordinary ground cost 1. Door open/closed state is automatic and authoritative: occupancy opens it and a short deterministic hold window closes it after passage. Unfinished blueprints remain passable. Stone walls and doors share the same presentation connectivity network, so a door visually interrupts and joins a continuous wall line rather than appearing as an unrelated prop. The Bevy client exposes Stone wall plus Door in the Build palette; Cancel-jobs removes unfinished sites but does not demolish completed structures. See [`ADR-0009`](../adr/0009-physical-construction-sites-and-blocking-structures.md).

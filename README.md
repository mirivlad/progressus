...
The sparse terrain state is not yet save/load data, a residency or unload policy, or a terrain gameplay command. Discovery is likewise an in-memory authoritative bootstrap: every character monotonically reveals a radius-five Euclidean disk of cells, while snapshots publish only `KnownTerrain` and player `MoveTo` cannot target or traverse unknown terrain. Continuous positions and bounded exact click-to-move are now an authoritative bootstrap, not complete navigation: there is no diagonal or hierarchical search, auto-repath, pawn collision, physical footprints, speed modifiers, save/load, or residency. Prototype harvest, haul, craft, and construction jobs now provide deterministic worker assignment, exclusive reservations, travel/work lifecycle, cancellation/manual-interruption cleanup, natural-source depletion, physical logistics, local recipe consumption, and delivered-material construction. Stockpiles remain physical floor areas rather than abstract containers. A bootstrap Workbench can craft `2 Wood + 1 Stone -> 1 PrimitiveTool` from reserved stacks lying in adjacent stockpile cells; the output is a normal physical stack and can be hauled. `StoneWall` construction reserves a concrete Stone stack, physically carries it beside the site, consumes exactly 2 Stone after work, and turns the site into an authoritative movement/A* blocker with the same stable ID. Workbench placement remains instantaneous; demolition, doors, containers, stack splitting, stockpile filters/priorities, and persistence remain incomplete. The current boundary keeps the visual client dependent on `progressus-app` without giving it ownership of simulation truth; Bevy converts exact snapshots to local presentation floats only after subtracting a nearby cell-center origin.

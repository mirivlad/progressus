...
- a first physical-item ownership bootstrap: deterministic Wood/Stone stacks use global stable Progressus IDs, exact fixed-point ground positions, explicit Ground/Carried location state, chunk-aware lookup, and atomic reach-checked pickup/drop transitions in the pure Rust simulation; explored ground stacks cross the detached app snapshot boundary and render as disposable Bevy item markers;
...
The sparse terrain state is not yet save/load data, a residency or unload policy, or a terrain gameplay command. Discovery is likewise an in-memory authoritative bootstrap: every character monotonically reveals a radius-five Euclidean disk of cells, while snapshots publish only `KnownTerrain` and player `MoveTo` cannot target or traverse unknown terrain. Continuous positions and bounded exact click-to-move are now an authoritative bootstrap, not complete navigation: there is no diagonal or hierarchical search, auto-repath, jobs/AI interruption policy, pawn collision, physical footprints, speed modifiers, save/load, or residency. The new item model is only the ownership foundation: natural resource sources are visible but cannot yet be designated or depleted; there are no harvesting/hauling jobs, stockpiles, containers, construction, crafting, stack splitting/merging, or persistence yet. The current boundary keeps the visual client dependent on `progressus-app` without giving it ownership of simulation truth; Bevy converts exact snapshots to local presentation floats only after subtracting a nearby cell-center origin.
...
It opens seed `0` as a 2D visual bootstrap with five snapshot-driven characters, four small Wood/Stone supply markers, and explored natural-resource markers, arrow-key movement and Space to stop Cora, and presentation-only pan/zoom. Terrain is queried for the camera viewport plus a small margin only when that window, the authoritative exploration revision, or the authoritative item/resource revisions change; undiscovered terrain remains an unknown background and camera movement never discovers it.

...
Sparse authoritative persistence is now implemented as versioned save format v1: untouched worldgen is regenerated from seed/version while exploration, terrain overrides, depleted resources, stable-ID entities/items/jobs, character satiety, production state, construction, exact positions, reservations, and active navigation are restored from explicit save DTOs. Discovery remains authoritative and monotonic: every character reveals a radius-five Euclidean disk of cells, snapshots publish only `KnownTerrain`, and player `MoveTo` cannot target or traverse unknown terrain. Continuous positions and bounded exact click-to-move are still a navigation bootstrap: there is no diagonal or hierarchical search, auto-repath, pawn collision, physical footprints, or speed modifiers. Raw deterministic chunks now use character-centered bounded residency; entity sleeping and Simulation LOD remain future scaling layers. Prototype harvest, haul, craft, and construction jobs provide deterministic worker assignment, exclusive reservations, physical logistics, local recipe consumption, and delivered-material construction. Stockpiles remain physical floor areas rather than abstract containers. A bootstrap Workbench can craft `2 Wood + 1 Stone -> 1 PrimitiveTool`; missing inputs are split from eligible stockpile stacks and physically delivered to its Input ports, while output appears at its Output ports and ordinary Haul returns it to storage. `StoneWall` construction physically receives and consumes Stone before becoming an authoritative movement/A* blocker. Workbench placement remains instantaneous; demolition, doors, containers, player-directed/general-purpose stack splitting, stockpile filters/priorities, autosaves, entity sleeping, and Simulation LOD remain incomplete. The client still depends only on `progressus-app` for authoritative state; Bevy converts exact snapshots to local presentation floats only after subtracting a nearby cell-center origin.
